...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-gradient-to-br from-white via-slate-50 to-blue-50 text-gray-900">
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-gray-200/70 bg-white/85 shadow-sm backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 sm:px-8">
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="relative py-2 text-sm font-semibold text-gray-700 transition hover:text-blue-600"
              >
                {item.name}
                <span className="absolute inset-x-0 bottom-0 h-0.5 origin-left scale-x-0 rounded-full bg-blue-600 transition-transform duration-300 hover:scale-x-100" />
              </Link>
            ))}
          ...
          <button
            ...
            className="relative z-50 flex h-11 w-11 items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-900 shadow-sm transition hover:border-blue-200 hover:bg-blue-50 lg:hidden"
          ...
          >
            <span className="relative block h-5 w-6">
              ...
              <span
                className={`absolute left-0 top-4 h-0.5 w-6 rounded-full bg-current transition-all duration-300 ${
                  menuOpen ? "top-2 -rotate-45" : ""
                }`}
              />
            </span>
        ...
        {/* Menu mobile */}
        <div
          className={`fixed inset-0 top-[76px] z-40 bg-white transition-all duration-300 lg:hidden ${
            menuOpen
              ? "visible translate-x-0 opacity-100"
              : "invisible translate-x-full opacity-0"
          }`}
        >
          <div className="flex h-[calc(100vh-76px)] flex-col overflow-y-auto px-6 py-8">
            <nav className="flex flex-col gap-2">
              {navigation.map((item, index) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center justify-between rounded-2xl border border-transparent px-5 py-4 text-lg font-bold text-gray-800 transition hover:border-blue-100 hover:bg-blue-50 hover:text-blue-600"
                  style={{
            ...
            <div className="mt-8 border-t border-gray-200 pt-8">
              <Link
                href="/devis"
                onClick={() => setMenuOpen(false)}
                className="flex w-full items-center justify-center rounded-2xl bg-blue-600 px-6 py-4 text-base font-bold text-white shadow-xl shadow-blue-600/20 transition hover:bg-blue-700"
              >
                Demander un devis
              </Link>
